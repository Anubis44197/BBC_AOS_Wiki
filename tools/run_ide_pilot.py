...
class IDEPilot:
    ...
    def run_scenario(
        ...
        print(f"\n[IDE PILOT] Running Scenario '{name}' (Iterations: {iterations})...")
        ...
        # Inject developer cancellation/rejection on iteration 50
        interruption_iteration = 50

        for i in range(iterations):
            trace_id = f"tr_{name}_{i}"
            replay_id = f"rp_{name}_{i}"

            is_interruption_run = (i == interruption_iteration)
            ...
            if status["status"] != "COMPLETED":
                print(f"[IDE PILOT] Orchestrator halted as expected. Status: {status['status']}")

            coder_res = status["outputs"].get("coder_result")
            verify_res = status["outputs"].get("verify_result")
            tester_res = status["outputs"].get("tester_result")
            ...
            if is_interruption_run:
                # Simulate developer rejection
                # LOW risk request gets auto-approved, so we explicitly set status PENDING to simulate interruption rejection
                req = approval_manager.get_status(app_res["approval_id"])
                req.status = "PENDING"
                approval_manager.reject(app_res["approval_id"])
                
                # Rollback execution to coder stage (purges checkpoints after coder)
                status = orchestrator.rollback_execution(trace_id, replay_id, "coder")
                print(f"[IDE PILOT] Interrupted! Execution rolled back to: {status['current_stage']}")
                
                # Resume execution and run to completion
                status = orchestrator.resume_execution(trace_id, replay_id, context_data)
                
                # Re-fetch coder & verify results
                coder_res = status["outputs"].get("coder_result")
                verify_res = status["outputs"].get("verify_result")
                
                # Re-request approval
                app_res = approval_manager.request_approval(
                    trace_id=trace_id + "_res",
                    replay_id=replay_id + "_res",
                    risk_level=risk_level,
                    commit_payload=coder_res,
                )

            # Programmatically approve if pending/escalated
        ...
        int_rate = interruption_recoveries
...
def main() -> None:
    ...
    metrics_path = os.path.join(pilot.workspace_dir, "tools", "ide_pilot_metrics.json")
