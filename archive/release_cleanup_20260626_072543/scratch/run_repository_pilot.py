"""Phase 12A - E2E Repository Pilot Runner

Runs 100 E2E loops for 4 scenarios on a dedicated sandbox copy of the
legacy repository 'BBC_MASTER_BBCMath-main'.
"""
...
class RepositoryPilot:
    ...
    def run_scenario(
        ...
        # Fresh setup of Sandbox filesystem
        self.prepare_sandbox()
        memory_manager = self.setup_memory_manager()
        
        # Setup Audit Logs and Managers
        ...
        # Baseline outputs for determinism check
        ...
        for i in range(iterations):
            ...
            # 4. Commit Gateway Execution (mutates sandbox)
            # Add approval_id to verification data
            ...
            # 5. Verify filesystem mutation occurred
            mutated = True
            ...
            # 6. Rollback commit (reverts sandbox mutation)
            ...
            # 7. Record baseline & determinism checks
            if i == 0:
                ...
                # Compare deterministic commit hash and approval hash
                if (
        ...
        # Clean sandbox after scenario completes
        self._delete_sandbox()
...
def main() -> None:
    ...
    metrics_path = os.path.join(pilot.workspace_dir, "scratch", "pilot_metrics.json")
